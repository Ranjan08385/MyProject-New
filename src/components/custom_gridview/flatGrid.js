...
class FlatGrid extends React.Component {
  ...
  renderRow({ rowItems, rowIndex, separators, isLastRow, itemsPerRow, rowStyle, containerStyle }) {
    ...
    console.log('additionalRowStyle', itemContainerStyle);
    console.log('rowStyle', containerStyle);

    return (
      <View style={[rowStyle, additionalRowStyle]}>
        {rowItems.map((item, i) => (
          <View key={`item_${rowIndex * itemsPerRow + i}`} style={[itemContainerStyle]}>
            {renderItem({
              item,
